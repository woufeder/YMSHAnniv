function initLab() {
  ...
  const pairResolveDelay = 750;
  const completionDelay = 180;
  ...
  const experimentPairs = [
    {
      ...
      matchLabel: "協助判斷溶液酸鹼性",
      matchType: "用途",
      note: "指示劑鹼性時呈桃紅，酸性或中性溶液中呈無色。",
    },
    {
      id: "microscope",
      icon: "fa-microscope",
    ...
    resolveTimeout = window.setTimeout(() => {
      if (isMatch) {
        ...
        firstCard.setAttribute(
          "aria-label",
          `實驗資料卡 ${Number(firstCard.dataset.index) + 1}`,
        );
        secondCard.setAttribute(
          "aria-label",
          `實驗資料卡 ${Number(secondCard.dataset.index) + 1}`,
        );
        combo = 0;
  ...
  function showCompletion() {
    ...
    const pairCount = experimentPairs.length;
    // 排除每次翻牌校對的強制停留，並保留每組四秒的實際操作時間。
    const playerSeconds = Math.max(
      0,
      totalSeconds - (moves * pairResolveDelay + completionDelay) / 1000,
    );
    const timePenalty = Math.max(
      0,
      Math.ceil((playerSeconds - pairCount * 4) / 20),
    );
    const gradeScore = moves + timePenalty;
    const sThreshold = pairCount + 4;
    const aThreshold = pairCount * 2;
    const bThreshold = pairCount * 3;
    const rank =
      gradeScore <= sThreshold
        ? "S"
        : gradeScore <= aThreshold
          ? "A"
    ...
    overlay.innerHTML = `
      <section class="complete-popup" role="dialog" aria-modal="true" aria-labelledby="completeTitle">
        ...
        <div class="complete-actions">
          <button class="complete-restart btn control-btn" type="button"><i class="fa-solid fa-rotate-left me-2"></i>重新校對</button>
          <button class="complete-back btn control-btn""><i class="fa-solid fa-map me-2"></i>返回地圖</button>
        </div>
      </section>
    `;
